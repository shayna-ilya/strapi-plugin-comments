...
const ReportsReviewTable = ({
  ...
  const handleItemSelectionChange = (selection, value) => {
    if (isArray(selection)) {
      ...
      onSelectionChange([...selectedItems, selection].filter(_ => !value ? selection !== _ : value));
    }
  };
          ...
          <Tr>
            <Th>
              <BaseCheckbox 
                ...
                onValueChange={(value) => handleItemSelectionChange(items.filter((_) => !_.resolved).map(_ => _.id), value)} />
            </Th>
            <Th>
